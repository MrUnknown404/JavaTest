...
import main.java.javatest.util.KeyInput;
...
public class JavaGameTest extends Canvas implements Runnable {
	...
	public JavaGameTest() {
		addKeyListener(new KeyInput(objectHandler));
		
		new Window(WIDTH, HEIGHT, "Project Cards!", this);
		
		objectHandler.addObject(new EntityPlayer((WIDTH / 2) - 32, (HEIGHT / 2) - 32));
	...
	public synchronized void start() {
		System.out.println(Console.getInfo(EnumWarningType.Info) + "Starting!");
	...
	public void run() {
		System.out.println(Console.getInfo(EnumWarningType.Info) + "Started Run Loop");
		...
		while (running) {
			...
			if (running) {
				...
				if (System.currentTimeMillis() - timer > 1000) {
					timer += 1000;
					System.out.println(Console.getInfo() + "FPS: " + frames);
					frames = 0;
				}
			} else {
				stop();
	...
	private void render() {
		...
		if (bs == null) {
			...
			System.out.println(Console.getInfo(EnumWarningType.Info) + "Started Render Loop");
			return;
		}
